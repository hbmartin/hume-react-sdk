...
const trackWeakMapPromise = <K extends object, V extends object>(
  ...
  const trackedValue = createValue(promise);
  map.set(key, trackedValue);
  const clearTrackedValue = () => {
    if (map.get(key) === trackedValue) {
      map.delete(key);
    }
  };
  // Attach both settlement handlers directly to the promise returned to
  // callers. Besides clearing the entry, this prevents an ignored scoped stop
  // from becoming an unhandled rejection.
  void promise.then(clearTrackedValue, clearTrackedValue);
  return promise;
};
...
interface PlayerStopOperation {
  joinedStop: Promise<void> | null;
  promise: Promise<void>;
...
const useSoundPlayerImplementation = (
  ...
  const playerResourceFailureDetails = useRef<
    ...
    () =>
      new FftStore((error, context) => {
        reportPlayerResourceFailure(
          context,
          error,
          playerResourceFailureDetails.current,
        );
      }),
  );

  const clearPlayerFftStore = useCallback(
    (message: string, additionalDetails?: Record<string, unknown>) => {
      ...
      const previousDetails = playerResourceFailureDetails.current;
      playerResourceFailureDetails.current = additionalDetails;
      try {
        fftStore.clear(message);
      } catch (error) {
        reportPlayerResourceFailure(message, error, additionalDetails);
      } finally {
        playerResourceFailureDetails.current = previousDetails;
      }
    },
  ...
  const playerInitializationSequence = useRef(0);
  const playerUnmountSequence = useRef(0);
    ...
    const retrying = Promise.resolve().then(async () => {
      ...
      try {
        while (retry.pendingResources.size > 0) {
          ...
          const resourcesJoiningDisposal = resourcesToRetry.filter(
            (resources) => playerResourceDisposals.current.has(resources),
          );
          const resourcesStartingRetry = resourcesToRetry.filter(
            (resources) => !playerResourceDisposals.current.has(resources),
          );
          // A resource only counts as attempted when this pass starts a fresh
          // disposal for it. Joining a disposal that originally retained the
          // context must be followed by a real close attempt after it settles.
          resourcesStartingRetry.forEach((resources) =>
            retry.attemptedResources.add(resources),
          );
          await Promise.all([
            resourcesJoiningDisposal.length > 0
              ? disposePlayerResourceBatch(resourcesJoiningDisposal).catch(
                  () => undefined,
                )
              : Promise.resolve(),
            resourcesStartingRetry.length > 0
              ? disposePlayerResourceBatch(resourcesStartingRetry).catch(
                  (error: unknown) => {
                    appendCleanupFailures(failures, error);
                  },
                )
          ...
          resourcesJoiningDisposal.forEach((resources) => {
            if (
              failedPlayerContextResources.current.has(resources) &&
              !retry.attemptedResources.has(resources)
            ) {
              retry.pendingResources.add(resources);
            }
          });
        }
        throwCleanupFailures(
  ...
  const initPlayer = useCallback(
    ...
    async (
      ...
      const initializationSequence = ++playerInitializationSequence.current;
      const initializationUnmountSequence = playerUnmountSequence.current;
      const preflightGeneration = playerGeneration.current;
      const requiresRetainedContextPreflight =
        sharedAudioContext === undefined &&
        failedPlayerContextResources.current.size >=
          MAX_RETAINED_PLAYER_CONTEXTS;
      if (requiresRetainedContextPreflight) {
        await retryFailedPlayerContextClosuresBestEffort();
        if (
          initializationSequence !== playerInitializationSequence.current ||
          preflightGeneration !== playerGeneration.current
        ) {
          return false;
        }
      ...
      if (!requiresRetainedContextPreflight) {
        void retryFailedPlayerContextClosuresBestEffort();
      }
        ...
        if (resources) {
          try {
            ...
            reportPlayerResourceFailure(
              ...
              error,
              initializationUnmountSequence === playerUnmountSequence.current
                ? undefined
                : { trigger: 'unmount' },
            );
  ...
  const startPlayerStop = useCallback(
    ...
    (request: PlayerStopRequest = {}): StartedPlayerStop => {
      ...
      // Preserve setup failures for the returned promise while still disposing
      // every captured resource below.
      try {
  ...
  const joinTrackedPlayerStop = useCallback(
    (operation: PlayerStopOperation, trigger: PlayerStopTrigger) => {
      if (operation.trigger === trigger || operation.scope === null) {
      ...
      const joinedStop = operation.promise.then(
        () => finishPlayerStopLifecycle(lifecycle),
        (error: unknown) => {
          const failures: unknown[] = [];
          appendCleanupFailures(failures, error);
      ...
      operation.joinedStop = joinedStop;
  ...
  const stopAllTracked = useCallback(
    (request: PlayerStopRequest = {}) => {
      ...
      const operation: PlayerStopOperation = {
        joinedStop: null,
        promise: stopping,
  ...
  const stopAllAndReportWithPrefix = useCallback(
    (failurePrefix: string, request: PlayerStopRequest = {}) => {
      ...
      if (existingReport) {
        // A report attached to the physical stop does not handle a rejecting
        // joined child promise. Observe that child before returning the shared
        // report so deduplicated reporting cannot create an unhandled rejection.
