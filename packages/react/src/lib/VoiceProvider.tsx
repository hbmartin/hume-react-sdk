...
type ForcedCleanupRequest = Readonly<{
  context: AudioContext | null;
  playerCleanupState: PlayerCleanupState | null;
...
export const VoiceProvider: FC<VoiceProviderProps> = ({
  ...
  const providerEffectActiveRef = useRef(true);
  ...
  const pendingDisconnectCleanupRef = useRef<PendingDisconnectCleanup | null>(
    null,
  );
  const pendingPlayerCleanupStatesRef = useRef(new Set<PlayerCleanupState>());
  ...
  const releasePlayerCleanupStateIfSettled = useCallback(
    (state: PlayerCleanupState) => {
      if (state.cleanupSettled && state.activeStop === null) {
        pendingPlayerCleanupStatesRef.current.delete(state);
      }
    },
    [],
  );
  ...
  const settlePlayerCleanup = useCallback(
    (state: PlayerCleanupState) => {
  ...
  const trackPlayerResourceCleanup = useCallback(
    <Result,>(
      ...
      const clearCleanup = () => {
        try {
          try {
            ...
            // Clear caller-owned locks before bookkeeping that could fail so a
            // settled cleanup can never deadlock a later disconnect.
            try {
              tracking.onSettled?.(trackedCleanup);
            } finally {
              if (tracking.playerCleanupState !== null) {
                settlePlayerCleanup(tracking.playerCleanupState);
              }
            }
          }
    ...
    [diagnostics, settlePlayerCleanup],
  );
  ...
  const runForcedCleanup = useCallback(
    async ({
      ...
      if (context) {
        if (playerCleanupState !== null && stillOwnsResources()) {
    ...
    onClose: useCallback(
      (
        ...
        const closeCleanupStillOwnsResources = () => {
          ...
          const disconnectOwner = pendingDisconnectCleanupRef.current?.owner;
          return (
        ...
        const playerCleanup = (async () => {
          ...
          await stopPlayerForCleanup(playerCleanupState, sharedContextToClose);
        })();
        ...
        trackedCloseCleanup = trackPlayerResourceCleanup(
          closeCleanup,
          'server_close',
          async () => {
            ...
            const forcedCleanup = await runForcedCleanup({
              context: sharedContextToClose,
              playerCleanupState,
              steps: microphoneStreamStopped
                ? []
                : [
                    {
              ...
              stillOwnsResources: closeCleanupStillOwnsResources,
              timeoutMessage,
            });
    ...
    async (options: ConnectOptions, connectRequestGeneration: number) => {
      ...
      const cleanupAttemptResources = (stopPlayer: boolean) => {
        const playerCleanupState = stopPlayer
          ? beginPlayerCleanup(
              providerEffectActiveRef.current ? {} : { trigger: 'unmount' },
            )
          : null;
        const cleanup = (async () => {
          const failures: string[] = [];
        ...
        return trackPlayerResourceCleanup(
          cleanup,
          'connection_attempt',
          async () => {
            const timeoutMessage = `Connection attempt cleanup exceeded ${RESOURCE_CLEANUP_TIMEOUT_MS} ms.`;
            const forcedCleanup = await runForcedCleanup({
              context: sharedCtx,
              playerCleanupState,
              steps: [],
              // This attempt permanently owns its captured context. Explicitly
              // scoped cleanup remains safe after a later attempt replaces the
              // provider's current context reference.
              stillOwnsResources: () => true,
              timeoutMessage,
            });
            return forcedCleanup.failures;
          },
          { playerCleanupState },
        );
      };
  ...
  const disconnectAndCleanUpResources = useCallback(
    (diagnosticReason: DisconnectDiagnosticReason = 'consumer') => {
      ...
      if (existingCleanup) {
        ...
        return existingCleanup.promise;
      }
      ...
      const cleanupStillOwnsLifecycle = () =>
        ...
        pendingDisconnectCleanupRef.current?.owner === cleanupOwner;
        ...
        try {
          ...
          if (sharedContextToClose) {
            await attempt('Audio player cleanup failed', () =>
              stopPlayerForCleanup(playerCleanupState, sharedContextToClose),
            );
            if (!rawCleanupStillOwnsLifecycle()) return;
      ...
      cleanup = trackPlayerResourceCleanup(
        rawCleanup,
        'connection',
        async () => {
          ...
          const forcedCleanup = await runForcedCleanup({
            ...
            stillOwnsResources: cleanupStillOwnsLifecycle,
            timeoutMessage: `Connection cleanup exceeded ${RESOURCE_CLEANUP_TIMEOUT_MS} ms.`,
          });
        ...
        },
        {
          ...
          onSettled: (settledCleanup) => {
            if (
              pendingDisconnectCleanupRef.current?.promise === settledCleanup
            ) {
  ...
  useEffect(() => {
    ...
    providerEffectActiveRef.current = true;
    // disconnect from socket when the voice provider component unmounts
    return () => {
      providerEffectActiveRef.current = false;
