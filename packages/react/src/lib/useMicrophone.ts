...
export const useMicrophone = (props: MicrophoneProps) => {
  ...
  const applyMuteStateToTracks = useCallback(
    (tracks: MediaStreamTrack[], muted: boolean) => {
      ...
      try {
        tracks.forEach((track) => {
          if (!enabledStates.has(track)) return;
  ...
  const applyMuteStateToActiveStreams = useCallback(
    ...
    (muted: boolean) => {
      ...
      if (!muted) {
        let pendingReconciliationRequired = false;
        try {
          ...
          if (pendingStream && pendingStream !== stream) {
            pendingReconciliationRequired = true;
            const pendingTracks = pendingStream.getAudioTracks();
            tracks.push(...pendingTracks);
          }
          applyMuteStateToTracks(tracks, false);
        } catch (error) {
          updatePendingReconciliation(pendingReconciliationRequired);
          throw error;
        }
  ...
  const reportMicrophoneResourceFailure = useCallback(
  ...
  const reportAnalyzerFailure = useCallback(
    (error: unknown, expectedDraw?: FrameRequestCallback) => {
      ...
      clearFftStore('Failed to reset FFT state after an analyzer failure.');
    },
    // oxlint-disable-next-line react/preserve-manual-memoization -- diagnostics is a stable latest-value ref whose current value must not trigger callback recreation
    [clearFftStore, diagnostics, stopFftAnalyzer],
  );
    ...
    (stream: MediaStream) => {
      ...
      const draw: FrameRequestCallback = () => {
        ...
        try {
          ...
          const frameGeneration = fftFrameGeneration.current;
          fftAnimationId.current = requestAnimationFrame((timestamp) => {
            if (
              frameGeneration !== fftFrameGeneration.current ||
              fftDrawCallback.current !== draw
            ) {
              return;
            }
  ...
  const disposeMicrophoneResources = useCallback(
    async (options: DisposeMicrophoneOptions = {}) => {
      ...
      stopFftAnalyzer();
      clearFftStore('Failed to clear FFT state during microphone cleanup.');
    ...
    (stream: MediaStream, sharedAudioContext?: AudioContext) => {
      ...
      try {
        if (isMutedRef.current) {
          ...
          clearFftStore(
            'Failed to clear FFT state while starting a muted microphone.',
          );
        }
  ...
  const performReplace = useCallback(
    ...
    async (
      ...
      const readRegisteredCandidate = (): PendingMicrophoneReplacement | null =>
      ...
      const registeredCandidate = readRegisteredCandidate();
      if (
      ...
      if (registeredCandidate.needsMuteReconciliation) {
        try {
          ...
          disposeCandidate();
          try {
            await disposeMicrophoneResources({ preserveMute: true });
          } catch (cleanupError) {
            reportClosureFailure(
  ...
  const mute = useCallback(
    () => {
      ...
      pauseFftAnalyzer();
      clearFftStore('Failed to clear FFT state while muting the microphone.');
      setIsMuted(true);
      diagnostics.current.emit({
        level: 'info',
        category: 'microphone',
