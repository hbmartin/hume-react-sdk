...
describe('useSoundPlayer', () => {
  ...
  let readAudioContextState: () => AudioContextState;
  ...
  beforeEach(() => {
    ...
    readAudioContextState = () => audioContextState;
  ...
  it.each([1, 2])(
    'fails safely when AudioContext state is unreadable on initialization read %s',
    async (throwOnRead) => {
      const stateError = new Error('context state unavailable');
      let stateReadCount = 0;
      readAudioContextState = () => {
        stateReadCount += 1;
        if (stateReadCount === throwOnRead) throw stateError;
        return 'suspended';
      };
      const onError = vi.fn();
      const { result } = renderHook(() =>
      ...
      expect(resumeAudioContext).toHaveBeenCalledTimes(throwOnRead - 1);
      expect(closeAudioContext).toHaveBeenCalledOnce();
      expect(onError).toHaveBeenCalledWith(
        expect.stringContaining(stateError.message),
        'audio_player_initialization_failure',
      );
    },
  );
  ...
  it('reports initialization and rollback cleanup failures separately', async () => {
    ...
    expect(onError.mock.calls[1]?.[0]).toContain(
      'Failed to load audio worklet; cleanup also failed:',
    );
    expect(
      events.find(
        (event) =>
  ...
  it('preserves the provider initialization reason when rollback cleanup fails', async () => {
    vi.mocked(loadAudioWorklet).mockResolvedValueOnce(false);
    const cleanupError = new Error('analyser disconnect failed');
    disconnectAnalyserNode.mockImplementationOnce(() => {
      throw cleanupError;
    });
    const events: VoiceDiagnosticEvent[] = [];
    const diagnostics = createVoiceDiagnosticsReporter(() => ({
    ...
    expect(cleanupEvent).toMatchObject({ category: 'audio_player' });
    expect(JSON.stringify(cleanupEvent?.details['error'])).toContain(
      cleanupError.message,
    );
  });

  it('retains context cleanup ownership when reading its state throws', async () => {
    const stateError = new Error('context state unavailable');
    const closeError = new Error('context close failed');
    const analyserError = new Error('analyser disconnect failed');
    closeAudioContext
    ...
    if (!context) throw new Error('Expected the first audio context.');
  ...
  it('does not block initialization on a detached context that cannot close', async () => {
    const closeError = new Error('context still cannot be closed');
    const onError = vi.fn();
    const { result } = renderHook(() =>
      useSoundPlayer({
        enableAudioWorklet: false,
  ...
  it('starts a fresh retry after joining a disposal that fails mid-pass', async () => {
    const joinedDisposal = createDeferred<void>();
    const firstCloseError = new Error('first context close failed');
    const joinedCloseError = new Error('joined context close failed');
    closeAudioContext
      .mockRejectedValueOnce(firstCloseError)
      .mockReturnValueOnce(joinedDisposal.promise)
      .mockResolvedValueOnce(undefined);
    const { result } = renderHook(() =>
      useSoundPlayer({
        enableAudioWorklet: false,
    ...
    let disposingDetachedContext = Promise.resolve();
    act(() => {
      disposingDetachedContext =
    ...
    joinedDisposal.reject(joinedCloseError);
    await expect(disposingDetachedContext).resolves.toBeUndefined();
    await waitFor(() => expect(closeAudioContext).toHaveBeenCalledTimes(3));
  ...
  it('bounds retained owned contexts when close keeps failing', async () => {
    ...
    expect(globalThis.AudioContext).toHaveBeenCalledTimes(2);
    expect(closeAudioContext).toHaveBeenCalledTimes(5);
  ...
  it('rechecks the retained-context limit after successful retries', async () => {
  ...
  const beginRetainedContextPreflight = async () => {
    ...
    return { ...rendered, events, preflightClose, staleInitialization };
  };
  ...
  it('abandons a retained-context preflight when the player unmounts', async () => {
    const { events, unmount, preflightClose, staleInitialization } =
      await beginRetainedContextPreflight();
  ...
  it('reports a joined unmount lifecycle when a retained-context preflight fails', async () => {
    const closeError = new Error('joined preflight close failed');
    const { events, unmount, preflightClose, staleInitialization } =
      await beginRetainedContextPreflight();

    act(() => unmount());
    await act(async () => {
      preflightClose.reject(closeError);
      await expect(staleInitialization).resolves.toBe(false);
    });
    await waitFor(() =>
      expect(
    ...
    expect(JSON.stringify(cleanupFailure?.details['error'])).toContain(
      closeError.message,
    );
  });
  ...
  it('refuses at the retained-context cap without resetting an active shared player', async () => {
    ...
    disconnectAnalyserNode.mockClear();
    disconnectGainNode.mockClear();
    onError.mockClear();
    ...
    await act(() =>
      result.current.addToQueue({
        id: 'shared-player-remains-active',
        index: 0,
        data: '\x01',
        type: 'audio_output',
        receivedAt: new Date(0),
  ...
  it('releases a retained context that closes after the timeout', async () => {
    vi.useFakeTimers();
    ...
    Object.defineProperty(firstContext, 'state', {
      configurable: true,
      get: () => firstContextState,
    });
    globalThis.AudioContext = vi
      .fn()
    ...
    let replacement = Promise.resolve(true);
    act(() => {
      replacement = result.current.initPlayer();
    });
    await act(() => vi.advanceTimersByTimeAsync(1_000));
  ...
  it('joins concurrent cleanup requests for the same context', async () => {
    ...
    act(() => {
      ...
      secondStop = result.current
        .stopAllForContext(context, { trigger: 'unmount' })
        .then(() => {
          secondSettled = true;
        });
    });
  ...
  it('joins a detached context stop after another context becomes active', async () => {
    vi.useFakeTimers();
    ...
    let firstStop = Promise.resolve();
    act(() => {
      firstStop = result.current.stopAllForContext(firstContext);
    });
    await act(() => result.current.initPlayer(undefined, secondContext));
    ...
    act(() => {
      attributedStop = result.current
        .stopAllForContext(firstContext, { trigger: 'unmount' })
        .then(() => {
    ...
    await act(async () => {
      ...
      await Promise.all([firstStop, attributedStop]);
    });
  ...
  it('disposes standalone player resources when unmounting', async () => {
    ...
    expect(
      events.find(
        (event) =>
          event.name === 'resource.stop_started' &&
          event.details['scope'] === 'active_player' &&
          event.details['trigger'] === 'unmount',
      ),
    ).toMatchObject({
      details: {
  ...
  it('reports one failure when unmount joins a public stop', async () => {
    ...
    vi.spyOn(globalThis, 'cancelAnimationFrame').mockImplementation(() => {});
    const events: VoiceDiagnosticEvent[] = [];
    const unmountFailureReported = createDeferred<void>();
    const diagnostics = createVoiceDiagnosticsReporter(() => ({
      level: 'debug',
      logger: false,
      onEvent: (event) => {
        ...
        if (
          ...
          unmountFailureReported.resolve();
        }
      },
    }));
    const onError = vi.fn();
    ...
    await act(async () => {
      await vi.advanceTimersByTimeAsync(100);
      await stopping;
      await unmountFailureReported.promise;
    });
    ...
    const publicStopFailure = events.find(
      (event) =>
        event.name === 'resource.cleanup_failed' &&
        event.details['operation'] === 'stop' &&
        event.details['trigger'] === undefined,
    );
    expect(publicStopFailure).toMatchObject({
      details: {
        message: 'Audio player cleanup failed.',
      },
    });
    expect(JSON.stringify(publicStopFailure?.details['error'])).toContain(
      'joined stop failed',
    );
    const joinedStopFailure = events.find(
      (event) =>
        event.name === 'resource.cleanup_failed' &&
        event.details['trigger'] === 'unmount',
    );
    expect(joinedStopFailure).toMatchObject({
      details: {
        joined: true,
        message: 'Failed to dispose audio player while unmounting.',
      },
    });
    expect(JSON.stringify(joinedStopFailure?.details['error'])).toContain(
      'joined stop failed',
    );
  });
  ...
  it('returns lifecycle setup failures as a promise and still disposes resources', async () => {
    ...
    await expect(stopping).rejects.toBe(lifecycleError);
    expect(disconnectAnalyserNode).toHaveBeenCalledOnce();
    expect(disconnectGainNode).toHaveBeenCalledOnce();
  });
  ...
  describe('waitForQueueToDrain', () => {
    ...
    it.each([true, false])(
      'wakes a drain and retries retained contexts after an implicit stop (worklet: %s)',
      async (enableAudioWorklet) => {
        const activeContextClose = createDeferred<void>();
        const events: VoiceDiagnosticEvent[] = [];
        const diagnostics = createVoiceDiagnosticsReporter(() => ({
          level: 'debug',
          logger: false,
        ...
        closeAudioContext
          .mockRejectedValueOnce(new Error('detached context close failed'))
          .mockRejectedValueOnce(new Error('detached context retry failed'))
          .mockReturnValueOnce(activeContextClose.promise)
          .mockRejectedValueOnce(new Error('stop-time retry failed'))
          .mockResolvedValueOnce(undefined);
        const { result, unmount } = renderHook(() =>
          useSoundPlayer({
            diagnostics,
        ...
        expect(closeAudioContext).toHaveBeenCalledTimes(4);

        await act(async () => {
          activeContextClose.resolve();
